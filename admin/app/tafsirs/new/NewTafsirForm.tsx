...
import {
  createTafsirScholar,
  TafsirScholarFormState,
} from "@/lib/tafsirs";
import { TextField } from "@/app/components/TextField";

const initialState: TafsirScholarFormState = {};

export function NewTafsirForm() {
  const router = useRouter();
  const [state, formAction, isPending] = useActionState(
    async (prevState: TafsirScholarFormState, formData: FormData) => {
      const result = await createTafsirScholar(prevState, formData);
      if (!result.error) {
        router.push("/tafsirs");
      }
      return result;
